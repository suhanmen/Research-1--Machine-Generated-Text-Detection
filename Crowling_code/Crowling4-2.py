...
def retry(ExceptionToCheck, tries=8, delay=3, backoff=2, logger=None):
...
@retry(Exception, tries=8, delay=2+random.random(), backoff=2)
def find_data(site):
    total_list = []
...
def save_results_to_txt(txt_list, output_directory, point):
    output_file_path = os.path.join(output_directory, f'result_{310+point}.txt')
...
## 실행
directory_path = r'C:\Users\SJK\Desktop\ToC-Work\1.research(1)\Task1\Crowling\extracted_files\sample_results\total_data\3.310~\data'
output_directory = r'C:\Users\SJK\Desktop\ToC-Work\1.research(1)\Task1\Crowling\extracted_files\sample_results\total_data\3.310~'


site_list = []
site_count = []
site_list = read_all_text_files(directory_path)
#site_list = site_list[56:]
point = 0
